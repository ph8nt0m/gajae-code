...
- Coordinator MCP stdio server now dispatches JSON-RPC requests with bounded concurrency and answers the standard `ping` utility, so a long-running tool call (e.g. `gjc_coordinator_await_turn`, which polls for minutes) no longer starves keepalive on the same channel — without the transport hazards unbounded concurrency would introduce. `ping` is a reserved control frame that bypasses the data-concurrency cap; data-handler fanout is bounded (excess rejected as `server_busy`); `writeLine` failures are terminal instead of poisoning the write chain; EOF drains in-flight handlers under a bounded timeout before returning; and byte chunks are decoded with a streaming decoder. Same-session mutation atomicity continues to be provided by the coordinator's existing session-state file locks, verified by concurrency regression tests asserting a single active turn survives.
